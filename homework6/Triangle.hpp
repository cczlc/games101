...
public:
    MeshTriangle(const Vector3f* verts, const uint32_t* vertsIndex, const uint32_t& numTris, const Vector2f* st)
    {
        ...
        memcpy(vertices.get(), verts, sizeof(Vector3f) * maxIndex);               // 找到最大下标并将顶点数据进行拷贝
        vertexIndex = std::unique_ptr<uint32_t[]>(new uint32_t[numTris * 3]);
        memcpy(vertexIndex.get(), vertsIndex, sizeof(uint32_t) * numTris * 3);    // 将顶点索引进行拷贝
        numTriangles = numTris;                                                   // 定义网格三角形的数量为2
