...
public:
    MeshTriangle(const std::string &filename)
    {
        ...
        for (int i = 0; i < mesh.Vertices.size(); i += 3)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                auto vert = Vector3f(mesh.Vertices[i + j].Position.X, mesh.Vertices[i + j].Position.Y, mesh.Vertices[i + j].Position.Z) * 60.f;
            ...
            new_mat->Kd = 0.6;         // 这里的color是一个定值
            new_mat->Ks = 0.0;
            new_mat->specularExponent = 0;       // 这里没有再考虑高光项
        ...
        // 将所有的三角形都放入这里面中
        // 所以模型内部也有一个加速结构（因此，在总的场景中，每个模型都有自己的包围盒，其次每个模型自己也有一个BVH）
    ...
    std::unique_ptr<Vector2f[]> stCoordinates; // 三角形的纹理坐标

    std::vector<Triangle> triangles; // 三角形序列
...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    if (fabs(det) < EPSILON)             // 这个值太小了（实际上对应什么物理情况？）
        return inter;
    ...
    u = dotProduct(tvec, pvec) * det_inv; // 不在三角形内
    if (u < 0 || u > 1)
        return inter;
